...
| Kill | `zellij action close-tab-by-id <id>` (tab id resolved fresh from the pane id when possible; teardown can pass recorded `zellij_tab_id` when the pane is already gone) | Unlike herdr (where closing a tab's only pane also closes the tab), closing a zellij pane with `close-pane` does NOT close the now-empty tab - it survives as an empty "ghost" entry in `list-tabs`. `close-tab-by-id` on a LIVE tab (with its pane still running) verified to cleanly remove both pane and tab in one call. Kill resolves the owning tab and closes by tab id; if the pane has already disappeared, teardown's recorded `zellij_tab_id` closes the ghost tab instead. Best-effort (`\|\| true`), matching tmux's `kill-window` and herdr's `pane close` contract. |
...
Submit `printf '__FM_ZELLIJ_CWD__:%s\n' "$PWD"` into the pane via the same `send_text_line` primitive used for `treehouse get` itself, briefly settle, capture the pane, and read only the marked output line.
This works because `$PWD` comes from the current foreground shell no matter how many subshells deep the pane is, sidestepping the need for any structured field at all.
The marker avoids false matches from absolute-path prompts, previous scrollback, and treehouse's own `~`-prefixed "Entered worktree at ..." banner (`tests/fm-backend-zellij.test.sh` pins both prompt-path and banner cases).
...
1. Send, capture, and cwd operations call `fm_backend_zellij_target_ready` first, which verifies session existence via the passive `list-sessions` check and verifies the specific terminal pane via `list-panes --json` filtered to `.id == <pane>` and `.is_plugin == false`.
   This catches a whole session gone (killed externally, or a stale meta from a prior run) and the normal stale-pane case before sending, capturing, or reading cwd.
   Kill checks the session, resolves the tab from the pane when possible, and uses teardown's recorded `zellij_tab_id` fallback when the pane is already gone so it can still close an empty ghost tab.
...
This is why `fm_backend_zellij_kill` resolves the owning tab id from the pane when possible, accepts teardown's recorded `zellij_tab_id` as a fallback when the pane has already gone, and calls `close-tab-by-id`, rather than mirroring herdr's simpler "close the pane, the tab follows" contract.
