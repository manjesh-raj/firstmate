...
1. Every op in `bin/backends/zellij.sh` calls `fm_backend_zellij_target_ready` first, which verifies session existence via the passive `list-sessions` check and verifies the specific terminal pane via `list-panes --json` filtered to `.id == <pane>` and `.is_plugin == false`.
   This catches a whole session gone (killed externally, or a stale meta from a prior run) and the normal stale-pane case before sending, capturing, reading cwd, or killing.
...
**Accepted residual gap**: a pane can still die in the brief window between `fm_backend_zellij_target_ready`'s `list-panes` check and the operation's own `zellij action` call.
...
- **A pane can still die after `target_ready` succeeds and before the operation runs.** `target_ready` now catches stale pane ids up front for every op, but zellij's unconditional exit 0 still leaves this narrow time-of-check/time-of-use race for one-shot operations (see "Unconditional exit code 0" above).
